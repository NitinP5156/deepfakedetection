        ...
        for i,frame in enumerate(self.frame_extract(video_path)):
            ...
            try:
                if len(faces) > 0:
                    ...
                    frame = frame[y:y+h, x:x+w]  # Crop the face region
            except:
                pass
    ...
    if request.method == "GET":
        ...
        if not model_name:
            return render(request, predict_template_name, {
                'error': 'No model found for the specified sequence length. Please try a different sequence length.'
            })
            
        path_to_model = os.path.join(settings.PROJECT_DIR, 'models', model_name)
        if not os.path.exists(path_to_model):
            return render(request, predict_template_name, {
                'error': 'Model file not found. Please ensure the model files are in the models directory.'
            })
        ...
        for i in range(sequence_length):
            ...
            x, y, w, h = faces[0]
            # Add boundary checks for padding
            top = max(0, y - padding)
            ...
            # Skip if the cropped face is too small
            if frame_face.size == 0 or frame_face.shape[0] < 10 or frame_face.shape[1] < 10:
                continue
